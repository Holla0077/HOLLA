...
export async function POST(req: Request) {
  try {
    ...
    const { walletId, amount, cardNumber, cardName, expiry } = body ?? {};
    ...
    if (wallet.asset?.code !== "GHS") return bad("Card withdrawal is only for GHS wallet", 400);

    // Atomic: deduct balance (only if sufficient) + create transaction
    const tx = await prisma.$transaction(async (db) => {
      const updated = await db.wallet.updateMany({
        where: { id: walletId, userId, balance: { gte: amountPesewas } },
